...
function [x,flag,relres,iter,resvec] = pcgmod(A,b,tol,maxit,M1,M2,x0,varargin)
...
r = b - A*x; %iterapp('mtimes',afun,atype,afcnstr,x,varargin{:});
...
%    normr = norm(b - iterapp('mtimes',afun,atype,afcnstr,x,varargin{:}));
   ...
   if (i < minit) && (normr <= tolb)               % check for convergence
      flag = 0;
      iter = i;
      break
   end
   ...
   if (i < minit)
      ...
      xmin = x;
      imin = i;
   elseif (normr < normrmin)           % update minimal norm quantities
      normrmin = normr;
      xmin = x;
      imin = i;
   end
